...
x_y = [(1,0),(1,1),(2,0),(2,1),(3,0),(3,1)]

probs = [0.24, 0.06, 0.18, 0.12, 0.08, 0.32]
N = 100000

samples = np.random.choice(len(x_y), size=N, p=probs)
#samples = [x_y[i] for i in samples_idx]
X = np.array([x_y[s][0] for s in samples])
Y = np.array([x_y[s][1] for s in samples])
#pairs = x_y #p.array(x_y)
#samples = pairs[np.random.choice(len(pairs), size=N, p=probs)]
